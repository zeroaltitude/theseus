...
**Ordering.** Hooks run **before final authorization**, never after it: `proposed call → hook transforms → schema validation → live policy and resource checks → confirmation bound to the final action → final revalidation → durable dispatch`. Any later change to arguments, target, tool, or policy context invalidates the confirm and reruns the checks. `ContextBuilt` additions go back through the budgeter, provenance labels, and compiler validation; `MessageSending` transforms cannot bypass disclosure checks; `PostToolCall.updated_output` never overwrites the immutable raw result or its success/failure status; a shell-backed hook skips recursive hook invocation but never policy or budget. Observer events are typed so that `continue: false` is not expressible from them; capabilities live in event-specific result types, not convention.
...
- **Edges (typed, directed, timestamped):** `next`, `replies_to`, `in_channel`, `by`, `about`, `derived_from`, `summarizes`, `supersedes`, `contradicts`, `same_entity`, `mentions_conversation`, `in_role`, `evidence_for`, `judged_by`, `part_of`.
- **Roots:** a node with no incoming `next`. A channel's first node is its first root. Compaction creates a new root: a `Summary` whose `summarizes` edges point at the range it stands in for, from which `next` continues. The range is not removed; it is simply outside the default view. The graph never loses a node.
...
Inbound `Message` nodes; the assembled-context manifest (node ids included, as a record, not text); model output as `Message`; `ToolCall`/`ToolResult`; loop `Judgment`s; `Task` mutations. The memory pass (§5) runs over these afterwards.
...
- The event record types: `Node`, `Edge`, `Execution`, `Action`, `Completion`, `JudgmentRecord`, `LedgerRow`, with schema version stamps and forward-only migration hooks.
...
- Hooks: `Gate`, `Transform`, `Claim`, `Observe` kinds with fail-closed gates, typed observer results, ordering before final authorization, ledger rows per invocation. Compiled-in plugins first; the WASM ABI is designed but not shipped.
...
- WASM plugin ABI shipped for hooks and tools.
